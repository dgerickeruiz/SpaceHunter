...
local titleLogo = display.newImageRect( "img/cclogo.png", 112, 76 )
titleLogo.x = display.contentCenterX
titleLogo.y = 225
titleLogo.alpha = 0

local sponsors = display.newImageRect( "img/sponsors.png", 141.1, 39.56 )
sponsors.x = display.contentCenterX +10
sponsors.y = 500
sponsors.alpha = 0

local frontlogo = display.newImageRect( "img/frontlogo.png", 172, 237.44 )
frontlogo.x = display.contentCenterX
frontlogo.y = 150
frontlogo.alpha = 0

local touchthescreen = display.newImageRect( "img/touchthescreen.png", 128.26, 16.85 )
...
local space = display.newImageRect(spacegroup, "img/space.png", 340, 720 )
space.alpha = 1

local mascara = graphics.newMask( "img/roketmask.png" ) -- 320.0, 1300.0
...
local nave = display.newImageRect( "img/nave.png", 145.6, 216.7 )
...
local function createlayer3() 
		...
		--ontouchl3:addEventListener("tap", gotoLayer3)
	end
...
local ontouchbuttondo = function(event)
	if ( event.numTaps == 1 ) then
		...
		transition.to( space, { time=3000, y=space.x+1280, onComplete = createlayer3} ) --gotoLayer3
		end
    else
        return false
    end
...
function scene:create( event )
	...
	local whitescreen = display.newImageRect( "img/white.png", display.actualContentWidth, display.actualContentHeight )
